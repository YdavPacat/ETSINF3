...
#+begin_src sql
SELECT g.cod_gen, g.nombre, COUNT(p.cod_peli), AVG(p.duracion)
FROM Genero g, Pelicula p
WHERE 5 < (SELECT COUNT(*) FROM Clasificacion c WHERE p.cod_peli = c.cod_peli)
...
#+begin_src sql
SELECT g.cod_gen, g.nombre, COUNT(p.cod_peli), AVG (p.duracion)
FROM Genero g, Pelicula p, clasificacion c1
WHERE 5 < (SELECT COUNT(*) 
           FROM Clasificacion c 
           WHERE c.cod_peli = p.cod_peli) 
    AND c1.cod_gen = g.cod_gen AND c1.cod_peli = p.cod_peli
GROUP BY g.cod_gen, g.nombre
ORDER BY g.nombre;
