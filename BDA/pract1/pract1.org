...
WHERE p.cod_pais = a.cod_pais AND a.cod_act = ac.cod_act AND 2 = (SELECT count(*)
                                                                  FROM actua ac2, actor a2
                                                                  WHERE a.cod_act = a2.cod_act AND ac2.cod_act = a.cod_act)
GROUP BY p.cod_pais, p.nombre
HAVING count(distinct ac.cod_act) = (SELECT MAX(count(distinct ac1.cod_act))
                                     FROM pais p1, actor a1, actua ac1
                                     WHERE p1.cod_pais = a1.cod_pais AND a1.cod_act = ac1.cod_act AND 2 = (SELECT count(*)
                                                                                                       FROM actua ac21, actor a21
                                                                                                       WHERE a1.cod_act = a21.cod_act AND ac21.cod_act = a1.cod_act)
                                     GROUP BY p1.cod_pais, p1.nombre)
...
*** 20. Obten el nombre de los amigos que han leído todas las obras del autor del indentidicador 'RUKI'
...
WHERE NOT EXISTS (SELECT *
                 ...
                 WHERE e.autor_id = 'RUKI'
                       AND e.cod_ob NOT IN (SELECT o.cod_ob
                                       FROM leer l, obra o
                                       WHERE  a.num = l.num 
...
WHERE EXISTS (SELECT *
              ...
              WHERE NOT EXISTS (SELECT *
                                FROM escribir e
                                WHERE e.autor_id = aut.autor_id 
                                  AND e.cod_ob NOT IN(SELECT l2.cod_ob
                                                      FROM leer l2
                                                      WHERE l2.num = a.num)) 
                     AND EXISTS (SELECT *
                                FROM escribir e
...
*** 23 
#+begin_src sql
SELECT a.nombre, aut.nombre
FROM amigo a, autor aut
               WHERE NOT EXISTS (SELECT *
                                FROM escribir e
                                WHERE e.autor_id = aut.autor_id 
                                  AND e.cod_ob NOT IN(SELECT l2.cod_ob
...
WHERE NOT EXISTS (SELECT *
                  FROM escribir e
                  WHERE e.autor_id = au.autor_id
                  AND e.cod_ob NOT IN (SELECT l.cod_ob
                                       FROM leer l
                                       WHERE a.num = l.num ))
    AND EXISTS (SELECT *
                FROM escribir e
    ...
    AND NOT EXISTS 
        ...
        FROM leer l
        WHERE l.num = a.num
        AND l.cod_ob NOT IN (SELECT e.cod_ob
                             FROM escribir e
                             WHERE e.autor_id = au.autor_id)
        )
    AND EXISTS 
...
FROM libro l, esta_en es
WHERE l.id_lib = es.id_lib AND l.titulo is not null
...
#+begin_src sql
SELECT aut.nombre
FROM autor aut, escribir es 
WHERE aut.autor_id = es.autor_id
GROUP BY aut.nombre, aut.autor_id
HAVING count(es.cod_ob) = (SELECT (MAX (count (es2.cod_ob)))
                           FROM autor aut2, escribir es2
                           WHERE aut2.autor_id = es2.autor_id
                           GROUP BY aut2.autor_id)
#+end_src

*** 34 
#+begin_src sql
SELECT aut.nacionalidad
FROM autor aut
GROUP BY aut.nacionalidad
HAVING count(aut.autor_id) = (SELECT MIN (count(aut.autor_id))
                              FROM autor aut
                              GROUP BY aut.nacionalidad)
...
WHERE a.num = l.num 
GROUP BY a.num, a.nombre
HAVING count(l.cod_ob) =(SELECT MAX (count(l.cod_ob))
                         FROM amigo a, leer l
                         WHERE a.num = l.num 
                         GROUP BY a.num, a.nombre)
