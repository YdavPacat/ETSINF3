...
(deffacts mapa
    ...
    (limites 8 5)
    ;; Atributos dinámicos
    (problema pos 2 3 latas l 3 3 l 3 1 l 6 4 hueco h 2 1 h 6 1 h 8 1 h 1 3 h 4 3 h 8 3 h 1 5 h 4 5 h 7 5 prof 0)
)
...
(defrule mov_der
    ...
    (limites ?lim_x ?lim_y)
    (test (<> ?x ?lim_x))
    (test (not (member$ (create$ h(+ ?x 2) ?y) $?z)))
...
(defrule mov_aba
	?f <- (problema pos ?x ?y latas $?z prof ?nivel)
    (test (<> ?y 1))
    (test (not (member$ (create$ h ?x (- ?y 2)) $?z)))
...
(defrule mov_lata_arr
    ...
    (test (not (member$ (create$ h ?x (+ ?y 2)) $?w)))              ;; ¿Hay un hueco después de la lata?
...
(defrule mov_lata_aba
    ?f <- (problema pos ?x ?y latas $?z l ?l_x ?l_y $?w prof ?nivel)
    (test (<> ?l_y 1))
    (test (not (member$ (create$ h ?x (- ?y 2)) $?w)))
    (test (not (member$ (create$ l ?x (- ?y 2)) $?z)))
    (test (not (member$ (create$ l ?x (- ?y 2)) $?w)))
...
(defrule mov_lata_der
    ...
    (limites ?lim_x ?lim_y)
    (test (<> ?l_x ?lim_x))
    (test (not (member$ (create$ h (+ ?x 2) ?y ) $?w)))
...
(defrule tirar_lata_arr
    (declare (salience 20))
    ?f <- (problema pos ?x ?y latas $?z l ?l_x ?l_y $?w hueco $?h1 h ?hx ?hy $?h2 prof ?nivel)
    (limites ?lim_x ?lim_y)
    (test (<> ?l_y ?lim_y))
    (test (and (= ?l_x ?x) (= ?l_y (+ ?y 1))))         ;; ¿La lata está arriba del robot?
    (test (and (= ?hx ?x) (= ?hy (+ ?y 2))))
    (test (< ?nivel ?*prof*))
=>
    (bind ?*nod-gen* (+ ?*nod-gen* 1))
    (assert (problema pos ?x (+ ?y 1) latas $?z $?w hueco $?h1 $?h2 prof (+ ?nivel 1)))
)
...
(defrule tirar_lata_der
    ...
    (test (and (= ?hx (+ ?x 2)) (= ?hy ?y)))
    (test (< ?nivel ?*prof*))
=>
    (bind ?*nod-gen* (+ ?*nod-gen* 1))
    (assert (problema pos (+ ?x 1) ?y latas $?z $?w hueco $?h1 $?h2 prof (+ ?nivel 1)))
)
...
(defrule fin
    ...
    ?f <- (problema pos $?a latas hueco $? prof ?nivel)
=>
    (printout T"Fin" crlf)
